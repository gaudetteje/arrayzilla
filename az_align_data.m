...
% az_align_data(fdir) recursively operates on all files in a given
% directory
% az_align_data(fname,offset) starts at the specified offset in bytes
% az_align_data(...,'auto') automatically fixes errors less than 99.9% in
%     the file length and deletes data after 0.1%.
%
% az_align_data() interactively prompts user for a file
%
...
% call recursively if a directory was specified
if isdir(fname)
    files = findfiles(fname,'.srz',Inf,true);
    fprintf('Found %d SRZ data files in the directory "%s"\n', numel(files), fname)
    for fNum = 1:numel(files)
        fprintf('\n###############################################################\n')
        fprintf('###  Processing "%s"\n',files{fNum})
        az_align_data(files{fNum},OPTIONS);
    end
    return
end
...
while ~feof(fid)
    ...
    if any(res ~= code)
        ...
        if strcmpi(OPTIONS,'auto')
            if (idx/fstats.bytes) < .999
                key = 'f';      % Fix if problem found at less than 99.9% of file size
            else
                key = 'd';      % Delete if problem found at end of data
            end
        else
...
%                             end
                            ...
                            fprintf('*******************\n');
                            az_align_data(fname,srcloc,OPTIONS);
                            fprintf('*******************\n');
